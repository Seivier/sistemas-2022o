...
make zip no corría los tests con round robin para tricore.  El problema fue
corregido.

Un datarace que podía arrojar desborde de pila o pila corrupta en raras
ocasiones fue corregido.

Novedades T4V2

¡Resuelto un error que hacía que make zip no incluyera npub.c con su tarea!
...
Resuelto un error que podía generar segmentation fault en raras ocasiones
...
Ya hay una plantilla de lo pedido en nKernel/npub.c.plantilla.  Además
ya hay una solución programada pero que no cumple los requisitos pedidos
porque usa mutex y condiciones.  Le servirá para entender el algoritmo.
Debe reprogramarlo sin usar mutex y condiciones, recurriendo a operaciones
como START_CRITICAL, END_CRITICAL, setReady, suspend, schedule, etc.
...
pub.c test-pub.c: El problema del pub en formato aplicación
